...
class DatabaseWebInterface(BaseHTTPRequestHandler):
    ...
    def handle_checkin(self, data):
        try:
            conn = sqlite3.connect('/root/hotel-management/data/storage/โรงแรม.db')
            cursor = conn.cursor()
            
            # 1. Insert Guest
            cursor.execute('''
                INSERT INTO ผู้เข้าพัก (ชื่อ, โทรศัพท์, ห้อง, วันเช็คอิน, สถานะ)
                VALUES (?, ?, ?, ?, 'เข้าพัก')
            ''', (data['name'], data['phone'], data['roomId'], datetime.now().strftime("%Y-%m-%d")))
            ...
            self.end_headers()
            self.wfile.write(json.dumps({"success": True, "guest_id": guest_id}).encode('utf-8'))
    ...
    def handle_checkout(self, data):
        try:
            ...
            # Get guest ID from room first
            cursor.execute("SELECT รหัสผู้เข้าพัก FROM ห้องพัก WHERE เลขห้อง = ?", (data['roomId'],))
            result = cursor.fetchone()
            
            if result and result[0]:
                guest_id = result[0]
                # 1. Update Guest status
                cursor.execute("UPDATE ผู้เข้าพัก SET สถานะ = 'เช็คเอาท์', วันเช็คเอาท์ = ? WHERE รหัส = ?", 
                             (datetime.now().strftime("%Y-%m-%d"), guest_id))
            
            # 2. Update Room status
            cursor.execute('''
                UPDATE ห้องพัก 
                SET สถานะ = 'ว่าง', รหัสผู้เข้าพัก = NULL, วันที่อัพเดท = CURRENT_TIMESTAMP
                WHERE เลขห้อง = ?
            ''', (data['roomId'],))
    ...
    def handle_room_update(self, data):
        try:
            ...
            cursor.execute('''
                ...
                SET ราคา = ?, ประเภท = ?
                WHERE เลขห้อง = ?
            ''', (data['price'], data['type'], data['roomId']))
...
<body class="bg-slate-50 font-sans text-slate-800">
    ...
    def serve_table_data(self, table_name):
        ...
        try:
            conn = sqlite3.connect('/root/hotel-management/system-backend/database/data/โรงแรม.db')
